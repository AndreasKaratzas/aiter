...
try:
    ...
    from aiter.tuning.search.gemm.opus import (
...
try:
    from aiter.tuning.search.gemm.hipblaslt import Gemm as HipblasltGemm
...
def get_native_gemm_funcs():
    from aiter.ops.gemm.tuned import is_skinny_default_shape, skinny_gemm, torch_gemm
...
def get_asm_kernels(file, is_shuffle=False, require_fp32=False):
    if not os.path.exists(file):
        print(f"ASM kernel list file not exist: {file}")
        return {}
    df = pd.read_csv(file)
    if (
        "supports_fp32" not in df
        or not pd.api.types.is_integer_dtype(df["supports_fp32"].dtype)
        or not df["supports_fp32"].isin((0, 1)).all()
    ):
        raise ValueError("BF16 assembly manifest requires supports_fp32 integer 0 or 1")
    if require_fp32:
        df = df[df["supports_fp32"] == 1]
    return (
...
class GemmA16W16Tuner(GemmCommonTuner):
    ...
    def _setup_specific_arguments(self):
        ...
        self.parser.add_argument(
            ...
            "hipblaslt tuning is also available standalone via aiter/tuning/search/gemm/hipblaslt_cli.py.",
        )

    def _clear_op_caches(self):
        from aiter.ops.gemm.tuned import get_GEMM_A16W16_config, get_GEMM_A16W16_config_
    ...
    def run_config(self, args):
        from aiter.ops.gemm.tuned import gemm_a16w16
    ...
    def _get_asm_tasks(
        ...
        asm_kernels = get_asm_kernels(
            asm_kernel_list_csv, is_shuffle, outdtype == dtypes.fp32 or has_bias
        )
