...
"""Dedicated hipBLASLt search command, using the library's existing tuner.

Errors propagate to the invoking pipeline. A failed GPU process is never
silently retried or reported as a successful tuning result.
"""
...
def get_dtype(dtype_str: str):
    ...
    if dtype_str.startswith("torch."):
        value = getattr(torch, dtype_str.removeprefix("torch."), None)
        if isinstance(value, torch.dtype):
            return value
    name = dtype_str.removeprefix("torch.")
    if name not in names:
        raise ValueError(f"unsupported BLAS tuning dtype: {dtype_str}")
    return names[name]
...
def main():
    from aiter.tuning.search.gemm.hipblaslt import GemmTuner
    ...
    if args.tp <= 0 or args.batch_size <= 0 or any(size <= 0 for size in args.nsets):
        raise ValueError("tensor parallelism, batch size and nsets must be positive")
    ...
    if not args.untune_file:
        ...
        for n in sorted(nsets):
            for m, k in mksets:
                gtuner.add_gemm(m, n, k, indtype=dtype if args.model_dir else indtype)
        with tempfile.TemporaryDirectory(prefix="aiter-hipblaslt-") as directory:
            args.untune_file = str(Path(directory) / "untuned.csv")
            gtuner.untunedf.to_csv(args.untune_file, index=False)
            gtuner.run(args)
    else:
        gtuner.run(args)
    return 0
