...
tree wins over any installed aiter):
    cd <repo> && PYTHONPATH=$PWD \\
        python3 -m aiter.tuning gemm.opus_bmm_mxscale -g 16 -m 1,16,64 -n 1024 -k 4096
...
SHIPPED_CSV = str(
    BuildContext.load().resource(
        "configs", "model_configs/dsv4_batched_gemm_a8w8_blockscale_mxscale_tuned.csv"
    )
)
DEFAULT_OUT = os.path.abspath("dsv4_bmm_mxscale_retuned.csv")
