...
This file is retained for two reasons only:
  ...
  2. **Source of truth for tune-time helpers**: candidate_kids_for_shape,
     ...
     _ensure_kids_compiled live here. They are imported by:
       - gradlib's GemmTuner (aiter/tuning/search/gemm/hipblaslt.py) for the
         production `--libtype opus` path,
       - this script's own `tune()` for the single-shape debug path.
     aiter/codegen/gemm/opus/instances.py only owns the data constants
...
def candidate_kids_for_shape(M, N, K, bias, cu_num):
    ...
    try:
        from aiter.codegen.gemm.opus.instances import kernels_list as _klist
        from aiter.jit.utils.chip_info import get_gfx_runtime
...
def _ensure_kids_compiled(candidate_kids):
    ...
    """
    from aiter.codegen.gemm.opus.instances import heuristic_kids_for_arch
    ...
    try:
        ...
        service = get_service()
        _prev_rebuild = service.set_rebuild(1, invalidate=True)
        ...
        try:
            ...
            service.mark_built("module_deepgemm_opus")
            ...
            service.set_rebuild(_prev_rebuild)
...
# lines from aiter.testing by de...
...
def _opus_run_perftest(
    ...
    **kwargs,
    ...
    """Drop-in replacement for aiter.testing.run_perftest with a CUDA-
...
def _install_opus_perftest_once():
    """Monkey-patch aiter.testing.run_perftest with our CUDA-graph +
    cuda.Event timing replacement. Runs exactly once per subprocess, the
    first time run_opus_gemm_bench is invoked.

    This is NOT a modification of aiter.testing or mp_tuner source
    ...
    try:
        import aiter.testing.measurement as _tc
