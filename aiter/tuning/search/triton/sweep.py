...
from .parameters import pre_pruning_rules, validate_configuration
...
def echo_to_file(msg: str, filename: str, clear: bool = False):
    with open(filename, "w" if clear else "a") as stream:
        stream.write(msg + "\n")


def date_to_file(filename: str):
    echo_to_file(datetime.now(timezone.utc).isoformat(), filename)
...
def parse_args():
    ...
    parser.add_argument(
        "F", type=str, help="registered profiling driver, such as gemm_a16w16"
    )
    parser.add_argument(
        "--block-size-m-range",
        nargs="+",
...
def main():
    ...
    K = args.K
    G = args.G
    driver = resolve(args.F)
    ut_filename = driver.name
    ...
    assert M == next_power_of_2(M), "M has to be power of 2"
    if min(M, N, K) <= 0 or G < 0 or batch_timeout <= 0:
        raise ValueError(
            "dimensions and timeout must be positive; GPU index must be nonnegative"
        )
    assert all(
        v == next_power_of_2(v) for v in block_size_m_range
    ), "All possible BLOCK_SIZE_M must be power of 2"
    assert all(
        v == next_power_of_2(v) for v in block_size_n_range
    ), "All possible BLOCK_SIZE_N must be power of 2"
    assert all(
        v == next_power_of_2(v) for v in block_size_k_range
    ...
    for spk in num_ksplit_range:
        if spk <= 0:
            raise ValueError("split-K search values must be positive")
    ...
    for config_list in parms_comb_list:
        validate_configuration(config_list)
    ...
    if comb_max_batch <= 0:
        raise ValueError("SCREEN_MAX_BATCH must be positive")
    date_to_file(log_filename)
    env = os.environ.copy()
    env.pop("ROCR_VISIBLE_DEVICES", None)
    env["HIP_VISIBLE_DEVICES"] = f"{G}"
    attempt_dir = Path(tempfile.mkdtemp(prefix=f"{file_tag}-attempts-", dir=Path.cwd()))
    ...
    while i_comb_start < len(parms_comb_list):
        ...
        cmd = [
            ...
            str(attempt_dir),
            "-o",
            f"res-{file_tag}-{i_comb_start}-{i_comb_end}",
            "--",
            *driver.command((M, N, K), comb_str.split()),
        ]

        rocprof_filename = str(
            attempt_dir / f"res-{file_tag}-{i_comb_start}-{i_comb_end}_kernel_trace.csv"
        )

        if os.path.isfile(rocprof_filename):
            Path(rocprof_filename).unlink()

        execution = run(cmd, environment=env, timeout=batch_timeout)
        stdout_data, stderr_data = execution["stdout"], execution["stderr"]
        ...
        attempt = {
            **execution,
            "configurations": parms_comb_list[i_comb_start:i_comb_end],
            "trace": rocprof_filename,
        }
        (attempt_dir / f"{i_comb_start}-{i_comb_end}.json").write_text(
        ...
        if not execution["timed_out"] and execution["returncode"] == 0:
            if os.path.isfile(rocprof_filename):
                cmd_rprof = [
                    sys.executable,
                    "-m",
                    "aiter.tuning.search.triton.profile",
                    rocprof_filename,
                    "-k",
                    "gemm",
                    "--output",
                    str(attempt_dir / f"{i_comb_start}-{i_comb_end}.samples.json"),
                ]
                ...
                if process.returncode == 0:
                    ...
                    successful_groups += number_of_kernel_runtime
    ...
    print(f"Retained trace attempts: {attempt_dir}")
    if successful_groups == 0:
        raise RuntimeError(
            "no configurations produced valid measurements; see retained attempts"
        )
