...
from aiter.codegen.moe.ck_two_stage.instances import (
...
from aiter.ops.moe.dispatch import (
...
from aiter.ops.quantization.packing import (
...
from aiter.tuning.search.moe.mxfp4 import (
...
class FmoeTuner(TunerCommon):
    ...
    def _clear_op_caches(self):
        import aiter.ops.moe.dispatch as fmoe_module
    ...
    def generate_data_2stages(
        ...
        if q_dtype_w == torch.int4 and q_type != QuantType.per_1x32:
            ...
            # matching tests/operators/hip/drivers/moe_2stage.py is_mxfp8 routing.
            w1_qt_shffle_ck = shuffle_weight_a16w4(w1_qt, 16, True)
            w1_scale_aiter = shuffle_scale_a16w4(w1_scale, expert, True)
            w2_qt_shffle_ck = shuffle_weight_a16w4(w2_qt, 16, False)
        ...
        if q_type == QuantType.per_1x32 and q_dtype_w == dtypes.i4x2:
            ...
            # replaced kernel -- MUST match tests/operators/hip/drivers/moe_2stage.py / production.
    ...
    def gen_flydsl_i4_2stages_task(self, info, blockMs):
        ...
        for blockM in blockMs:
            ...
            for kname, kparams in flydsl_s2_kernels.items():
                ...
                # tuner can emit a row that fails operators/test_moe_2stage.
                s2_tile_m = kparams["tile_m"]
                if s2_tile_m != blockM:
                    continue
                if kparams.get("mode", "atomic") != "atomic":
    ...
    def run_config(self, args):
        from aiter.ops.moe.dispatch import fused_moe, fused_topk
        ...
        for i in range(len(untunedf)):
            ...
            try:
                ...
                # Match the production/test path used by tests/operators/hip/drivers/moe_2stage.py.
                w1_qt_fmoe = w1_qt
                w2_qt_fmoe = w2_qt
                ...
                if q_type == QuantType.per_1x32 and q_dtype_w == dtypes.i4x2:
                    ...
                    # runtime-effective one (mirror tests/operators/hip/drivers/moe_2stage.py).
                    ...
                    # tests/operators/hip/drivers/moe_2stage.py is_mxfp8).
                    w1_qt_fmoe = shuffle_weight_a16w4(w1_qt_fmoe, 16, True)
                    w1_scale_fmoe = shuffle_scale_a16w4(w1_scale, expert, True)
                    w2_qt_fmoe = shuffle_weight_a16w4(w2_qt_fmoe, 16, False)
                    w2_scale_fmoe = fp4_utils.e8m0_shuffle(w2_scale)
                ...
                # path above consumes the int8 w*_qt). Mirrors tests/operators/hip/drivers/moe_2stage.py.
                ...
                if out.count_nonzero() == 0 and ref.count_nonzero() > 0:
                    ...
                    # vs the higher-precision reference. tests/operators/hip/drivers/moe_2stage.py
