...
def compile_kernel(args: CompileArgs):
    ...
    from aiter.aot.source import load_kernel_module

    mod = load_kernel_module(args.path)
    ...
    from aiter.aot.source import native_scalar_type

    def ty_to_cpp(type_name):
        return native_scalar_type(
            type_name, triton.runtime.driver.active.map_python_to_cpp_type
        )

    params = {
        "kernel_name": func_name,
        "triton_kernel_name": ccinfo.name,
        "bin_size": len(asm),
    ...
    from aiter.aot.source import write_launcher_sources

    output_files = write_launcher_sources(
        params, target.backend, f"{sig_hash}_{suffix}", out_name, args.out_path
    )
