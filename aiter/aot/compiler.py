...
def get_git_commit_id_short():
    """Return source provenance when the declared metadata root is a Git checkout."""
    try:
        commit_id = (
            subprocess.check_output(
                [
                    "git",
                    "-C",
                    str(BuildContext.load().resource("metadata")),
                    "rev-parse",
                    "--short",
                    "HEAD",
                ],
...
AITER_NATIVE_DIR = str(BUILD_CONTEXT.resource("native"))
AITER_ASSEMBLY_DIR = str(BUILD_CONTEXT.resource("kernels", required=False))
AITER_PYTHON_ROOT_DIR = str(BUILD_CONTEXT.package.parent)
AITER_PACKAGE_DIR = str(BUILD_CONTEXT.package)
...
AITER_ROOT_DIR = os.environ.get(
    "AITER_AOT_CACHE_DIR",
    os.environ.get("AITER_ROOT_DIR", str(cache_directory() / "aot")),
)
os.environ["AITER_AOT_CACHE_DIR"] = AITER_ROOT_DIR
...
CK_DIR = str(BUILD_CONTEXT.resource("ck", required=False))
...
def mp_lock(
    ...
    """Serialize AOT compilation using the shared build-lock lifecycle."""
    from aiter.jit.utils.file_baton import FileBaton, run_with_baton

    return run_with_baton(FileBaton(lock_path), main_func, final_func, wait_func)
...
def transfer_hsaco(hsaco_path):
    ...
    hsaco = catalog.read(str(Path(hsaco_path).resolve().relative_to(catalog.root)))
...
def get_hsaco_launcher(hsaco_name, kernel_name):
    from aiter.ops._native.hsaco_launcher import HsacoLauncher, read_hsaco
