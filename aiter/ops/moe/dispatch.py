...
from aiter.jit.core import AITER_CONFIGS, PY, bd_dir, mp_lock
...
#     aiter/codegen/moe/mxfp4_aux.py) and an un-codegen'd
...
def fused_moe(
    ...
    if (
        ...
        from aiter.ops.moe.flydsl import _fhmoe
...
def _validate_selected_ck_layout(metadata, w1, w2, quant_type):
    """Validate the selected CK adapters before sorting or allocating intermediates."""
    from aiter.ops.moe_op import _validate_ck_moe_weight_layout

    stage1 = getattr(metadata.stage1, "func", metadata.stage1)
    stage2 = getattr(metadata.stage2, "func", metadata.stage2)
    if stage1 is ck_moe_stage1:
        _validate_ck_moe_weight_layout(
            w1.dtype, quant_type, getattr(w1, "is_shuffled", False)
        )
    if stage2 is aiter.ck_moe_stage2_fwd:
        _validate_ck_moe_weight_layout(
            w2.dtype, quant_type, getattr(w2, "is_shuffled", False)
        )
...
def _fused_moe_impl(
    ...
    *,
    ...
    _validate_selected_ck_layout(metadata, w1, w2, quant_type)
...
def get_2stage_cfgs(
    ...
    def MainFunc():
        ...
        os.system(
            f"{PY} -m aiter.tuning moe.two_stage -i {untune_file} -o {tune_file} -o2 {profile_file} --last"
        )
