...
import aiter
from aiter.aot.compiler import (
    AITER_NATIVE_DIR,
    AITER_PACKAGE_DIR,
    compile_template_op,
...
from aiter.aot.gluon.compiler import (
    CompileGluonArgs,
    compile_gluon_kernel,
)
from aiter.aot.triton.compiler import (
    CompileArgs,
    compile_kernel,
)
from aiter.ops._native.torch_utils import torch_to_c_types
...
def compile(
    ...
    if not_built(func_name):
        ...
        from aiter.aot.compiler import BUILD_DIR

        aot_file_dir = str(Path(BUILD_DIR) / "generated" / func_name)
        os.makedirs(aot_file_dir, exist_ok=True)

        compile_args = CompileGluonArgs(
            path=f"{AITER_PACKAGE_DIR}/ops/triton/gluon/pa_decode_gluon.py",
        ...
        def main_func():
            ...
            with open(
                f"{AITER_NATIVE_DIR}/cpp_itfs/pa_gluon_aot/pa_decode_attention_reduce_kernel.cpp.jinja",
                "r",
            ) as f:
        ...
        if main_func_result is not None:
            ...
            clean_aot_temporary_files_cmd = ["rm", "-rf", aot_file_dir]
