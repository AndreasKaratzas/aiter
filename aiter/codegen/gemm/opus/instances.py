...
#
# SCOPE (on-hardware validated, see operators/test_opus_gfx1250_ws.py -- 156/156):
...
CO_KERNELS_JSON = str(BuildContext.load().resource("native", "opus_gemm/gen_co/co_kernels.json"))
...
def _load_co_kernels(path, require_image=True):
    ...
    if missing:
        print(
            ...
            + ". Build them with aiter/codegen/gemm/opus/build_codeobjects.py --llvm-bin "
            "<pin-capable LLVM>.",
            file=sys.stderr,
        )
    return out
