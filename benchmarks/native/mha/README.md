# Native attention benchmarks

These programs measure the existing C++ attention entry points, `aiter::mha_fwd`, `aiter::mha_fwd_splitkv` and `aiter::mha_bwd`. They include CPU numerical references. They are separate from the public C ABI SDK and do not imply a prepared attention plan.

## Build and run

Run from the repository root in the selected ROCm environment. Choose a new output directory; an existing directory is rejected so stale libraries cannot satisfy the build.

```sh
python -m benchmarks.native.mha.build bwd_v3 \
  --architecture gfx950 --output /tmp/attention-bwd

python -m aiter.kernels run --target gfx950 -- \
  /tmp/attention-bwd/bwd.exe -prec=bf16 -b=1 -h=2 -s=64 -s_k=64 \
  -d=64 -d_v=64 -v=1 -warmup=1 -repeat=2
```

A successful numerical check prints `valid:y` and separate dQ, dK and dV reference statistics. Keep validation enabled when changing the implementation. Timings describe the selected kernel workload, not model throughput.

The builder accepts `fwd`, `bwd`, `fwd_v3`, `bwd_v3` or `all`. `fwd` and `bwd` include CK; `bwd_v3` uses the assembly path and a CK-free benchmark host. `fwd_v3` still uses CK headers in its benchmark host. `--architecture` selects the target for both the library and host. Use `ROCM_PATH` to select the ROCm toolchain; `--compiler` overrides only the host compiler. The small `build_mha.sh` wrapper accepts the same arguments.

Each invocation compiles into a private `.jit` cache inside the chosen output directory, then copies the requested library beside `fwd.exe` or `bwd.exe`. An executable's relative runtime search path finds that adjacent library. `build.json` retains commands, the selected AITER package, output hashes and build status; individual build logs are retained even after a failure. Every requested library and executable must be present and nonempty before the build is marked successful. Source, installed package and benchmark directories remain input locations.

For the larger existing smoke scripts, set `AITER_MHA_FWD_EXE` or `AITER_MHA_BWD_EXE` to the exact executable, then invoke the selected script with Bash. They no longer search the working tree for an arbitrary executable.

## Link an existing attention entry point

To produce only the shared library:

```sh
python -m benchmarks.native.mha.compile --api bwd_v3 \
  --architecture gfx950 --output /tmp/attention-library
```

Link the resulting library using its explicit output path. The checked host command in `build.py` shows the required native and optional CK include directories, resolved through `BuildContext`. These specialized C++ entry points retain their existing layout and build dependencies. For the versioned C SDK and its installed CMake package, start with [the SDK guide](../../../include/aiter/README.md).
