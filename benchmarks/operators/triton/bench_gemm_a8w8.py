...
from benchmarks.common.argparse import (
    add_argparse_ff,
    get_ff_args,
    get_parser,
)
...
def run_benchmark(args, defaults):
    if args.shape and args.model:
        raise ValueError("--shape and --model are mutually exclusive")
    if args.gluon:
        if args.shuffle:
...
def parse_args(args=None):
    ...
    return get_ff_args(parser, args)


def main(argv=None):
    args, defaults = parse_args(argv)
    if args.print_vgpr:
