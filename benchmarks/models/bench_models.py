...
from .catalog import KERNEL_DICT, load_kernel
...
class MoeKernelHandler(KernelHandler):
    ...
    def parse_stdout(self, stdout: str) -> float:
        ...
        data_line = lines[-1]
        graph = re.search(
            r"\btotal\s+([\d.eE+-]+)us\s+([\d.eE+-]+) TF/s\s+([\d.eE+-]+) TB/s",
            data_line,
        )
        if graph:
            latency, flops, bandwidth = map(float, graph.groups())
            return {
                "time": latency * 1e-3,
                "throughput": flops,
...
def read_json(json_path: str) -> ModelShapesData:
    full_path = files("benchmarks.models").joinpath(json_path)
...
def print_and_save_results(
    ...
    output_path = Path(f"{output_file}.csv")
