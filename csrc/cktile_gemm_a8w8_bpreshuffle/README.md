...
`FLATMM_HIP_CLANG_PATH=/data/llvm-project/build/bin/ python3 -m aiter.tuning gemm.a8w8_bpreshuffle -i aiter/configs/a8w8_bpreshuffle_untuned_gemm.csv -o aiter/configs/a8w8_bpreshuffle_tuned_gemm.csv --libtype cktile`
...
3. Test the performance, modify the test instance in `tests/operators/hip/drivers/gemm_a8w8.py` and run it, please wait a few minutes as it will build gemm_a8w8_bpreshuffle_cktile kernels in `aiter/configs/a8w8_bpreshuffle_tuned_gemm.csv` via jit：
`FLATMM_HIP_CLANG_PATH=/data/llvm-project/build/bin/ python3 tests/operators/hip/drivers/gemm_a8w8.py`
