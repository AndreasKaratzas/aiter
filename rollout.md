...
## GitHub directory migration

Review the [workflow navigation change](notes.md#september-7-github-workflow-navigation) as one connected migration. Canonical YAML and directory guides move to `.github/workflow-sources/`; the Python generator stays in `ci/workflows/`. The execution directories are `repository`, `library`, `frameworks`, `release` and `reusable`, with a matching `schedules` hierarchy. Shared steps live in `.github/actions/common/`. The [GitHub guide](.github/README.md) explains where a newcomer should start.

Land renamed sources, generated entrypoints, reusable calls, script callers, ownership rules and the migration map together. Generation and Actionlint must pass, and QA must account for all 58 workflows, 134 job identifiers and 19 cron selections. Preserve the `Checks` gate and `Documentation` publication name. A workflow filename rename creates a new GitHub file identity; saved manual links and any external automation referring to old filenames must use the migration map. Review configured branch rules when adopting the change outside this fork.

Publish the strict-built, browser-checked documentation with the new directory guides. This migration does not activate default-branch schedules, add GPU coverage or promote release artifacts. The profile names and their selected tests remain unchanged.
