...
Strategy:
  ...
  * Run pa_fwd_asm on a single sequence whose 16 page entries repeat
    each chosen block in turn, with context_lens = 16 × block_size.
  * Because the chosen block is filled with a constant V, the attention
    output equals that constant at every element (softmax weights sum
    to 1, and all referenced V slots have the same value).
...
Run:
    pytest /root/aiter/tests/operators/hip/test_pa_block_id_truncation.py -v -s

Or as a script:
    python /root/aiter/tests/operators/hip/test_pa_block_id_truncation.py
"""
...
def _pertoken_quant_kvcache_symm(k_cache, v_cache, quant_dtype):
    ...
    tests/operators/hip/drivers/pa_mtp.py."""
...
def _asm_V_shuffle(VC):
    ...
    expects. Mirrors tests/operators/hip/drivers/pa_mtp.py:asm_V_shuffle."""
...
def _run_pa_fwd_asm(
    ...
    """Run pa_fwd_asm with one sequence whose pages repeat `target_block_id`.
    Return the full attention output so every element can be checked.
...
def _assert_block_id_fingerprint(actual, expected_sig, block_id, kv_quant, tag):
    """Every output element must match the selected block's fingerprint."""
    msg = (
        f"[{tag}] block_id={block_id}: every output element must equal "
        f"{expected_sig} within absolute tolerance {_ABS_TOL[kv_quant]}. "
    )
    if block_id >= 65_536:
    ...
    assert actual.numel() > 0, msg
    torch.testing.assert_close(
        actual,
        torch.full_like(actual, expected_sig),
        rtol=0,
        atol=_ABS_TOL[kv_quant],
        msg=lambda detail: msg + "\n" + detail,
    )


def test_block_id_fingerprint_rejects_cancelling_errors():
    """Equal and opposite corruptions must fail even when the mean is correct."""
    block_id, expected_sig = 65_536, 0.4
    k_cache, v_cache, k_scale, v_scale = _build_kv_cache("bf16", "noquant")
    actual = _run_pa_fwd_asm(k_cache, v_cache, k_scale, v_scale, block_id)
    _assert_block_id_fingerprint(
        actual, expected_sig, block_id, "noquant", "uncorrupted"
    )

    corrupted = actual.clone()
    corrupted.view(-1)[0] += 0.5
    corrupted.view(-1)[1] -= 0.5
    # This is precisely the aggregate check that previously hid these errors.
    assert corrupted.mean().item() == pytest.approx(
        expected_sig, abs=_ABS_TOL["noquant"]
    )
    with pytest.raises(AssertionError, match="every output element"):
        _assert_block_id_fingerprint(
            corrupted, expected_sig, block_id, "noquant", "corrupted"
        )
...
if __name__ == "__main__":
    ...
    def _report(
        ...
        for block_id, expected, label in _FINGERPRINTS:
            ...
            status = "OK" if error <= _ABS_TOL[kv_quant] else "BUG"
            print(
                f"[{status}] block_id={block_id:>7d}  expected={expected:.4f}  "
                f"range=[{actual.min().item():.4f}, {actual.max().item():.4f}]  "
                f"max error={error:.4f}  ({label})"
            )
