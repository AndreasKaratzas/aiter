...
Examples:
    python3 tests/operators/hip/test_mha_varlen_large_kv.py
    python3 tests/operators/hip/test_mha_varlen_large_kv.py -n 8 -s 2200000 -c 0 1
    python3 tests/operators/hip/test_mha_varlen_large_kv.py -n 1 -s 16777216 -c 0  # single head
    python3 tests/operators/hip/test_mha_varlen_large_kv.py -n 8 -s 900000 -c 0    # fails: span < 4GiB
"""
...
def _flops_bytes(S, nheads, nheads_k, dq, dv, causal, elem_size):
    # 2 flops per MAC, over QK^T (dq deep) and PV (dv deep); matches tests/operators/hip/test_mha.py.
