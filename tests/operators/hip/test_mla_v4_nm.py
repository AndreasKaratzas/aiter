...
Usage:
  pytest -xvs tests/operators/hip/test_mla_v4_nm.py
"""
...
import aiter
import aiter.ops.attention.mla  # main no longer auto-imports submodules; need explicit
from aiter import dtypes
from aiter.jit.utils.chip_info import get_gfx
from aiter.testing import checkAllclose, run_perftest

# ---------------------------------------------------------------------------
# Variant under test (matches the cfg_mla_v4_asm entry in
# kernels/gfx950/mla_v4/mla_v4_asm.csv served by csrc/py_itfs_cu/asm_mla_v4.cu).
# ---------------------------------------------------------------------------
...
def test_v4_nm_kernarg_scalar_slots(capfd, monkeypatch):
    ...
    aiter.ops.attention.mla.mla_decode_fwd_v4_nm(**args)
...
# docstring). Mirrors operators/rui.py's torch reference and tests/operators/hip/drivers/mla.py's
...
def _native_to_2buff_for_asm(input_bf16):
    ...
    NOTE: differs from operators/rui.py which writes 7 e8m0 bytes once. The
...
def _asm_attn_decode_bf16(
    ...
    `aiter.ops.attention.mla.mla_decode_fwd_v4_nm`, and reduce/reshape the FP32 split
    ...
    logits, attn_lse = aiter.ops.attention.mla.mla_decode_fwd_v4_nm(
        q=q_packed,
...
def _build_bf16_inputs(
    ...
    # Bare randn (~N(0,1)), matching tests/operators/hip/drivers/mla.py's input convention.
...
def _run_one_point(
    ...
    if num_kv_splits is None:
        ...
        num_kv_splits, _ = aiter.ops.attention.mla.get_meta_param(
            None,
            batch,
            batch * kv_seq_lens,
            num_heads,
    ...
    _ret, us_asm_total = run_perftest(
        aiter.ops.attention.mla.mla_decode_fwd_v4_nm,
        q=q_packed,
        qrope=q_rope.contiguous(),
        kv_buffer=kv_packed,
        kvrope=kv_rope.contiguous(),
...
def test_v4_nm_accuracy_and_perf():
    """Run the asm kernel via aiter.testing.run_perftest at a fixed
...
def _run_varlen_point(kv_lens, gqa_ratio=128, seed=0, attn_sink=True):
    ...
    logits, _ = aiter.ops.attention.mla.mla_decode_fwd_v4_nm(
        q=qp,
        qrope=qr.contiguous(),
        kv_buffer=kp,
...
def test_v4_nm_multi_split():
    ...
    logits, _ = aiter.ops.attention.mla.mla_decode_fwd_v4_nm(**args)
    torch.cuda.synchronize()
    ...
    out_ign, _lse_ign = aiter.ops.attention.mla.mla_decode_fwd_v4_nm(**args_ign)
    torch.cuda.synchronize()
...
def test_v4_nm_sink():
    ...
    aiter.ops.attention.mla.mla_decode_fwd_v4_nm(**args_a)
    torch.cuda.synchronize()
    ...
    aiter.ops.attention.mla.mla_decode_fwd_v4_nm(**args_b)
    torch.cuda.synchronize()
    out_b = args_b["output"]
    ...
    aiter.ops.attention.mla.mla_decode_fwd_v4_nm(**args_inf)
    aiter.ops.attention.mla.mla_decode_fwd_v4_nm(**args_big)
    torch.cuda.synchronize()
    ...
    with pytest.raises(ValueError, match="sink.*FP32|sink.*float32"):
        aiter.ops.attention.mla.mla_decode_fwd_v4_nm(**args_bad_dtype)
    ...
    with pytest.raises(ValueError, match="sink.*numel"):
        aiter.ops.attention.mla.mla_decode_fwd_v4_nm(**args_under)
    ...
    with pytest.raises(ValueError, match="sink.*numel"):
        aiter.ops.attention.mla.mla_decode_fwd_v4_nm(**args_over)
    ...
    with pytest.raises(ValueError, match="sink.*contiguous"):
        aiter.ops.attention.mla.mla_decode_fwd_v4_nm(**args_strided)
    ...
    with pytest.raises(ValueError, match="sink.*device|same device"):
        aiter.ops.attention.mla.mla_decode_fwd_v4_nm(**args_bad_device)
...
def _run_oob_guardpage_worker(worker_args, fault_label):
    ...
    env["AMD_LOG_LEVEL"] = "0"
    # The test harness already supplies the selected package and suite paths;
    # child workers inherit that exact environment.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "determinism / kernarg suite, invoke `pytest tests/operators/hip/test_mla_v4_nm.py`\n"
            "directly."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "kernels/gfx950/mla_v4/mla_v4_asm.csv ships a single (gqa=16, qSeqLen=4) "
