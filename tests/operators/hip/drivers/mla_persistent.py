...
def check_fold_seqlen_indptr_cuda_graph_capture():
    ...
    with torch.cuda.stream(warmup_stream):
        aiter.ops.attention.mla._fold_seqlen_indptr(indptr, 3)
...
def test_mla(
    ...
    # num_page = kv_indptr[-1].item()
    # kv_indices = torch.randperm(num_page, dtype=torch.int)
    # from aiter.ops.attention.native import decode_update_mla_metadata_v1
    ...
    def test_absorb_decode_ds32_opus():
        ...
        (_attn_logits, _attn_lse), us_asm_decode = run_perftest(
            aiter.ops.attention.mla.mla_decode_fwd_ds32,
            q_nope_buffer_fp8,
            q_rope_buffer_bf16,
            kv_nope_buffer_fp8,
            kv_rope_buffer_bf16,
