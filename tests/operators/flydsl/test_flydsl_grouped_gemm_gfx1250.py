...
"""gfx1250 grouped MoE GEMM tests through ``aiter.ops.moe.dispatch``.
...
(``python tests/operators/flydsl/test_flydsl_grouped_gemm_gfx1250.py``) runs a
...
# Build every tensor straight on the device (like tests/operators/hip/drivers/moe_2stage.py) so
...
# AITER_MOE_EXPERT_BALANCE=1 (mirrors tests/operators/hip/drivers/moe_2stage.py).
...
# Production MoE accuracy gate (matches tests/operators/hip/drivers/moe_2stage.py calc_diff):
...
def _torch_moe_ref(
    ...
    *,
    ...
    """Two-stage MoE reference reusing ``aiter.ops.moe.dispatch.torch_moe_stage{1,2}``."""
...
def _make_topk(
    ...
    tests/operators/hip/drivers/moe_2stage.py). ``AITER_MOE_NUM_EXPERT_ACTIVATED=n`` (highest
...
def _logits_diff(actual: torch.Tensor, expected: torch.Tensor) -> float:
    """MoE accuracy metric from tests/operators/hip/drivers/moe_2stage.py (calc_diff):
...
def run_moe(
    ...
    *,
    ...
    (tests/operators/hip/drivers/moe_2stage.py).  rel_l2 (~= sqrt(2*logits_diff)) is printed
...
def summarize(rows: list):
    ...
    Mirrors the pandas DataFrame reporting in tests/operators/hip/drivers/moe_2stage.py.
