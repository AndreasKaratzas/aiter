...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--script-path", default=".github/scripts/product/split_tests.sh")
...
def main() -> int:
    ...
    aiter_run_ids = fetch_recent_run_ids(
        args.repo, "product-legacy.yaml", args.runs_count
    )
    triton_run_ids = fetch_recent_run_ids(
        args.repo, "product-triton.yaml", args.runs_count
    )
    if not aiter_run_ids:
        raise RuntimeError(
            "No eligible completed (success/failure) runs found for product-legacy.yaml"
        )
    if not triton_run_ids:
        raise RuntimeError(
            "No eligible completed (success/failure) runs found for product-triton.yaml"
        )
