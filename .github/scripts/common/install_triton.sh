#!/bin/bash
set -euo pipefail
AITER_REQUIREMENTS_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../requirements" && pwd)"
...
if ! install_triton_from_wheelhouse "${TRITON_WHEEL_DIR}"; then
    ...
    python3 -m pip install --extra-index-url "$TRITON_INDEX_URL" -r "$AITER_REQUIREMENTS_ROOT/runtime/triton.txt"
fi
