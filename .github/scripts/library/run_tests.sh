...
export PYTHONPATH="$(pwd)/tests:$(pwd)${PYTHONPATH:+:$PYTHONPATH}"
...
skip_tests=(
    "benchmarks/communication/bench_mega_moe_v2.py"
    "tests/integration/communication/test_dispatch_combine.py"
    "tests/integration/communication/test_communication.py"
    "tests/integration/communication/test_mori_all2all.py"
    "tests/integration/communication/test_fused_ar_rms.py"
    "tests/integration/communication/test_mega_moe_v2.py"
    "tests/integration/communication/triton_test/test_reduce_scatter_all_gather.py"
    "tests/integration/communication/triton_test/test_fused_rs_rmsnorm_quant_ag.py"
)
...
for file in "${sharded_files[@]}"; do
    ...
    case "$file" in
        tests/integration/communication/test_mega_moe_gfx1250.py)
            {
            ...
            ;;
        tests/operators/hip/drivers/mla_persistent.py|tests/operators/hip/drivers/mla_persistent_round_robin.py)
            {
            ...
            ;;
        tests/operators/hip/test_gemm_a6w6.py)
            {
...
for f in "${sharded_files[@]}"; do
    if [[ "$f" == "tests/operators/hip/drivers/mla.py" ]]; then
        mla_in_shard=true
        break
    fi
done
if [[ "$mla_in_shard" == "true" && "$MULTIGPU" != "TRUE" ]]; then
    for args in \
        ...
        if ! timeout 10m python3 tests/operators/hip/drivers/mla.py $args 2>&1 | tee -a latest_test.log; then
            testFailed=true
