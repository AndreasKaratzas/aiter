...
@functools.cache
def root_dir() -> Path:
    return check_dir(Path(__file__).resolve().parents[4])
...
def list_triton_source_files() -> (
    ...
    test_dir = check_dir(root_dir() / "tests" / "operators" / "triton")
    bench_dir = check_dir(root_dir() / "benchmarks" / "operators" / "triton")
...
class Visitor(ast.NodeVisitor):
    ...
    def add_dependency(self, import_: str, suppress_warning: bool = False) -> None:
        ...
        namespace = import_.split(".", 1)[0]
        base = (
            root_dir() / "tests"
            if namespace
            in {"operators", "integration", "frameworks", "support", "unit"}
            else root_dir()
        )
        p = base / import_py_file
        if p.exists() and p.is_file():
            self.dependencies.add(p.relative_to(root_dir()))
            return
        # Check if it's a package directory with `__init__.py`.
        p_dir = (base / import_py_file).with_suffix("")
...
def build_dependency_graph(
    ...
    deps_to_ignore: set[Path] = {
        ...
        root_dir() / "aiter" / "ops" / "_native" / "hsaco_launcher.py",
    }
    assert all(
