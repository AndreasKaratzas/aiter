...
def list_test_files(repo_root: Path, test_type: str) -> list[str]:
    if test_type == "aiter":
        glob_pattern = "tests/operators/hip/test_*.py"
    else:
        glob_pattern = "tests/operators/triton/**/test_*.py"
    files = sorted(
...
def guess_test_file_from_testcase(
    ...
    if classname:
        ...
        if candidate in known_files:
            return candidate
        if "tests/" + candidate in known_files:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--script-path", default=".github/scripts/library/split_tests.sh")
...
def main() -> int:
    ...
    aiter_run_ids = fetch_recent_run_ids(
        args.repo, "library-legacy.yaml", args.runs_count
    )
    triton_run_ids = fetch_recent_run_ids(
        args.repo, "library-triton.yaml", args.runs_count
    )
    if not aiter_run_ids:
        raise RuntimeError(
            "No eligible completed (success/failure) runs found for library-legacy.yaml"
        )
    if not triton_run_ids:
        raise RuntimeError(
            "No eligible completed (success/failure) runs found for library-triton.yaml"
        )
